...
class Environment:
    ...
    def kinematicConstrant(self, q, qBefore, qNext):
        ...
        # Constrain the heading angle x   xres is the constrained heading angle
        deltax1x2 = self.angleVec(q2qNext[0:2], qBefore2q[0:2])  # Use dot product divided by the product of magnitudes to find the angle between the projections in the xoy plane
        ...
        # Constrain the climb angle gam   Note: the climb angle is only discussed in the range of -pi/2 to pi/2, which happens to be the same as the range of arcsin.  gamres is the constrained climb angle
    ...
    def angleVec(vec1, vec2):  # Calculate the angle between two vectors.
        ...
        temp = np.clip(temp, -1, 1)  # There may be precision errors that cause the previous temp to be slightly greater than 1, so clip it.
    ...
    def load_model(method):
        import sys
        sys.path.append('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/IIFDS-{}-random_start'.format(method)) # Resolve the issue where the model save path differs from the load file path.
...
if __name__ == "__main__":
    ...
    # Calculate adaptive weight related parameter assignment
    R=3
    gamma=0.99
    a_length=3
    K_1=1/4
    ...
    for step in range(500):
        ...
        # Threat coefficient calculation
        if data_dic['d'] <= 0:
            threat_index = np.inf
        elif data_dic['d'] <= 0.4:
