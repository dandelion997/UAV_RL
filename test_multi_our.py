...
class Environment:
    ...
    def kinematicConstrant(self, q, qBefore, qNext):
        ...
        # Constrain heading angle x   xres is the constrained heading angle
        ...
        # Constrain climb angle gam   Note: the climb angle is only discussed in the range of -pi/2 to pi/2, which happens to be the same as the value range of arcsin.  gamres is the constrained climb angle
    ...
    def load_model(method):
        import sys
        sys.path.append('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/IIFDS-{}-random_start'.format(method)) # Solve the problem that the saved model path is not the loading file path
...
if __name__ == "__main__":
    ...
    # Calculate relevant parameters for adaptive weighting
    R=3
    gamma=0.99
    a_length=3
    K_1=1/4
    ...
    for step in range(500):
        ...
        if if_test_origin_ifds:
            ...
            # Calculate optimal learning factor
