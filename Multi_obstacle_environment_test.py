...
class Environment:
    ...
    def reset(self):
        ...
        for i in range(self.obs_num):              # Clear all obstacle path records
            self.path[i] = np.array([[]]).reshape(-1, 3)
    ...
    def kinematicConstrant(self, q, qBefore, qNext):
        ...
        # Constrain the heading angle x   
        deltax1x2 = self.angleVec(q2qNext[0:2], qBefore2q[0:2])  # Calculate the angle between projections on the xoy-plane using the dot product divided by the product of magnitudes.
        ...
        # # Constrain the climb angle gam. Note: The climb angle only needs to be considered in the interval [-pi/2, pi/2], which coincides with the range of arcsin. gamres is the constrained climb angle.
    ...
    def initField(self, pos, V0, goal):
        """Calculate the initial flow field and return a column vector."""
        temp1 = pos[0] - goal[0]
        temp2 = pos[1] - goal[1]
        temp3 = pos[2] - goal[2]
    ...
    def trans(self, originalPoint, xNew, yNew, zNew):
        """
        Coordinate transformation to the Earth coordinate system
    ...
    def angleVec(vec1, vec2):  # Calculate the angle between two vectors
        ...
        temp = np.clip(temp, -1, 1)  #There may be precision errors causing temp to be slightly greater than 1, so use clip here
    ...
    def partialDerivativeSphere(obs, pos, r):
        """Compute the partial derivatives of the spherical obstacle equation and return a column vector."""
        temp1 = pos[0] - obs[0]
        temp2 = pos[1] - obs[1]
        temp3 = pos[2] - obs[2]
    ...
    def load_model(method):
        import sys
        sys.path.append('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/IIFDS-{}-random_start'.format(method)) # Handle the case where the model save path is different from the load file path.
...
if __name__ == "__main__":
    ...
    uav_pos = env.start
    uav_path = env.start.reshape(1,-1)   # Record the UAV trace array
    actionCurve = np.array([])
    action_trace = np.array([]).reshape(-1,3)  # Record the action trajectory array (three actions just fit n*3 shape)
